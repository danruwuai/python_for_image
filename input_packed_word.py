...
def get_packed_word_file(dir_path):
    ...
    for root, dirs, files in os.walk(dir_path):  # 遍历所有子目录        
        ...
        # 获取文件名
        file_list.extend(os.path.join("", file) for file in files if (file.endswith('.packed_word') and '-yplane-' not in file and 'cplane' not in file and os.path.isfile(file)))
        # print(file_list)
    return file_list
...
def get_packed_word_s0_file(dir_path):
    ...
    for root, dirs, files in os.walk(dir_path):  # 遍历所有子目录
        ...
        # file_list.extend(os.path.join(root, file) for file in files if file.endswith("packed_word"))
        # 获取文件名
        file_list_s0.extend(os.path.join("", file) for file in files if (file.endswith("packed_word") and "_s0." in file and "yplane" in file and os.path.isfile(file)))
        # print(file_list_s0)
    return file_list_s0
...
def get_lsc_raw_file(dir_path):
    ...
    for root, dirs, files in os.walk(dir_path):  # 遍历所有子目录 
        ...
        # file_list.extend(os.path.join(root, file) for file in files if file.endswith("packed_word"))
        # 获取文件名
        file_list_lsc.extend(os.path.join("", file) for file in files if (file.endswith(".raw") and "_LSC_" in file and "_16_" in file and os.path.isfile(file)))
