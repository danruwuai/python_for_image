...
def do_aao(frame_data, height, width, current_height, name, flag):
    ...
    if flag == 2:
        ...
        csv_data[100, 0] = 'G_AVG_12bit'
        G_AVG_data = G_byte
        G_AVG_data = G_AVG_data.astype('str')
        csv_data[101: current_height + 101, 0: width] = G_AVG_data
        csv_data[200, 0] = 'B_AVG_12bit'
        B_AVG_data = B_byte
        B_AVG_data = B_AVG_data.astype('str')
        csv_data[201: current_height + 201, 0: width] = B_AVG_data
    """
    # 显示函数直接显示效果
    ...
    if flag == 1 or flag == 2:
        ...
        if flag == 2:
            csv_data[300, 0] = 'R_SUM_18bit'
            R_SUM_data = R_byte
            R_SUM_data = R_SUM_data.astype('str')
            csv_data[301: current_height + 301, 0: width] = R_SUM_data
            csv_data[400, 0] = 'G_SUM_18bit'
            G_SUM_data = G_byte
            G_SUM_data = G_SUM_data.astype('str')
            csv_data[401: current_height + 401, 0: width] = G_SUM_data
            csv_data[500, 0] = 'B_SUM_18bit'
            B_SUM_data = B_byte
            B_SUM_data = B_SUM_data.astype('str')
            csv_data[501: current_height + 501, 0: width] = B_SUM_data
            # 替换csv_data数据里的-1为空
            csv_data[csv_data == '-1'] = ''
